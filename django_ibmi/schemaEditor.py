...
class DB2SchemaEditor(BaseDatabaseSchemaEditor):
    ...
    sql_alter_column_type_from_int_to_auto = "ALTER COLUMN %(column)s SET GENERATED BY DEFAULT AS " \
                                             "IDENTITY( START WITH %(max)d, INCREMENT BY 1, CACHE 10 ORDER )"
    ...
    def alter_field(self, model, old_field, new_field, strict=False):
        ...
        if (old_db_field_type != new_db_field_type) and (isinstance(old_field, (models.AutoField, models.TextField))
                                                         or isinstance(new_field, models.TextField)):
            ...
            old_field, new_field = self.alterFieldDataTypeByRemaking(
        ...
        if alter_field_primary_key and old_field.primary_key:
            if strict:
                pk_names = self._constraint_names(
                    model, [old_field.column], primary_key=True)
                if len(pk_names) == 0:
                    raise ValueError("Found no primary key in %s.%s " % (
        ...
        if alter_field_unique and old_field.unique or (old_field.unique and alter_field_primary_key
                                                       and not old_field.primary_key):
            unique_key_names = self._constraint_names(
                model, [old_field.column], unique=True)
            if strict and len(unique_key_names) != 1:
                raise ValueError("Found wrong number of unique constraints for "
                                 "%s.%s" % (model._meta.db_table, old_field.column))
        ...
        if alter_field_index and old_field.db_index:
            ...
            if strict and len(index_names) != 1:
                raise ValueError("Found wrong number of Indexes for "
                                 "%s.%s" % (model._meta.db_table, old_field.column))
        ...
        if alter_field_check_constraint and old_db_field['check']:
            check_constraint_names = self._constraint_names(
                model, [old_field.column], check=True)
            if strict and len(check_constraint_names) != 1:
                raise ValueError(
                    "Found wrong number of check constraints for "
                    "%s.%s" % (model._meta.db_table, old_field.column))
        ...
        if alter_field_name or alter_field_data_type:
            ...
            if old_field.primary_key and new_field.primary_key:
                ...
                for incoming_fks in old_field.model._meta.get_fields():
                    fk_names = self._constraint_names(
                        incoming_fks.model, [incoming_fks.field.column], foreign_key=True)
                    for fk_name in fk_names:
                        self.execute(
                            self.sql_delete_fk % {
            ...
            self._defer_constraints_check(constraints, deferred_constraints, old_field, new_field,
                                          model, defer_pk=True, defer_unique=True, defer_index=True, defer_check=True)
            ...
            self._restore_constraints_check(
                deferred_constraints, old_field, new_field, model)
        ...
        for inc_rel in incoming_relations:
            fk_db_field = inc_rel.field.db_parameters(
    ...
    def alterFieldDataTypeByRemaking(self, model, old_field, new_field, strict):
        ...
        tmp_new_field.column = truncate_name("%s%s" % (
            self.psudo_column_prefix, tmp_new_field.column), self.connection.ops.max_name_length())
    ...
    def add_field(self, model, field):
        ...
        if notnull or unique or p_key:
            del_column = self.sql_delete_column % {'table': self.quote_name(
                model._meta.db_table), 'column': self.quote_name(field.column)}
            if notnull:
                field.null = False
                sql = self.sql_alter_column_not_null % {
                    'column': self.quote_name(field.column)}
                sql = self.sql_alter_column % {'table': self.quote_name(
                    model._meta.db_table), 'changes': sql}
                try:
                    self.execute(sql)
                    self._reorg_tables()
                except Error as e:
            ...
            if p_key:
                ...
                constraint_name = self._create_index_name(
                    model, [field.column], suffix="_uniq")
                sql = self.sql_create_unique % {'table': self.quote_name(
                    model._meta.db_table), 'name': constraint_name, 'columns': self.quote_name(field.column)}
                try:
                    self.execute(sql)
                    self._reorg_tables()
                except Error as e:
    ...
    def _alter_many_to_many(self, model, old_field, new_field, strict):
        ...
        if((old_field.remote_field is not None and hasattr(old_field.remote_field, 'through')) and
            ...
            rel_new_field = new_field.remote_field.through._meta.get_field(
